...
"""
    ...
    def process(self,block):
        ...
        for row in block:
                ...
                if x > self.pidmaxpide[pid]: self.pidmaxpide[pid]=x
    ...
    def finalise(self):
        ...
        self.counts={} # bidirectional best hits, key = species
        self.total={}
        for pid in self.pidmaxpide:
                species=self.pid2species[pid]
                if not species in self.counts:
                        self.counts[species]=[0]*101
                x=self.pidmaxpide[pid]
                self.counts[species][x]+=1
        for species in self.counts:
                tot=0
                x=self.counts[species]
                for i in range(0,101): tot+=x[i]
                self.total[species]=tot
                #print >> sys.stderr, "total:", species, tot
        ...
        for species in self.counts:
                ...
                datarow=[str(float(w)/100),str(tot),str(ntarget),str(multiplicity),kingdom,genus,species,str(float(tot)/self.nprot),str(float(w)/100/tot),str(float(median)/100),",".join(str(a) for a in x),line]
                self.summary_data.append_row(datarow)
        self.summary_data.sort_block(0,reverse=True)
        self.summary_data.output(result=True)
